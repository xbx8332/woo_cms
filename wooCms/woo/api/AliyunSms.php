...
require_once ROOT_PATH . 'include' . DS . 'Aliyun_sms_sdk' . DS . 'vendor' . DS . 'autoload.php';
...
{    
    public function __construct()
    {
        ...
        $accessKeyId = trim(setting('sms_keyid'));##############
        
        $accessKeySecret = trim(setting('sms_keysecret'));###############  
        
        if (empty($accessKeyId) || empty($accessKeySecret)) {
            exception('请在“系统设置”中配置短信接口');
            exit;
        }      
