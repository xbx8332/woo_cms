...
class Form
{
    ...
    public function ueditor($name, $infos = array())
    {
        $info = $this->getInfo($name, $infos);
        $value = $this->getValue($info['nameStr']);       
        $value = $info['value'] ? $info['value'] : $value;

        return sprintf("<script name=\"%s\" id=\"%s\" %s type=\"text/plain\">%s</script>", $info['nameStr'], $info['id'], $info['else'], $value);
    }
    ...
    public function select($name, $options = array(), $infos = array())
    {
        $info = $this->getInfo($name, $infos); 
        $value = $this->getValue($info['nameStr']);        
        $value = $info['value'] !== false ? $info['value'] : $value;
        ...
        foreach ($options as $level_key => $level) {
            if (is_array($level)) {
                ...
                foreach ($level as $level_key1 => $level1) {
                    $optStr .= sprintf("\r\n\t<option value=\"%s\" %s>%s</option>", $level_key1, strval($level_key1) === strval($value) ? 'selected=""' : '', strval($level1));
                }
            } else {
                $optStr .= sprintf("\r\n\t<option value=\"%s\" %s>%s</option>", $level_key, strval($level_key) === strval($value) ? 'selected=""' : '', $level);
            }
        }
