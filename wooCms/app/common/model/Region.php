...
class Region extends App
{
    ...
    public function beforeWriteCall()
    {
        ...
        if (isset($this['title']) && isset($this['first_letter'])) {
            if (!trim($this['first_letter'])) {
                $this['first_letter'] = strtoupper(get_first_letter($this['title']));
            }
        }
        return $parent_rslt;
    }
    ...
    public function afterDeleteCall()
    {
        $parent_rslt = call_user_func(array('parent', __FUNCTION__));
        if (isset($this['id'])) {                   
            $this->where('id', 'IN', $this->getChildrenIds($this['id'], 0))->delete();
        }   
        return $parent_rslt;
    }
}
