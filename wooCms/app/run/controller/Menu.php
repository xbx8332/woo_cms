...
class Menu extends Run
{
    ...
    public function lists()
    {
        if ($this->args['list'] == 'list') {
            ...
            call_user_func(array('parent', __FUNCTION__));
            $this->addAction("树形列表", array('Menu/lists', ['parent_id' => 1]), 'fa-eye', 'layui-btn-normal');
        } else {
            $this->addAction("新增一级{$this->mdl->cname}", array('Menu/create', ['parent_id' => 1]), 'fa-plus-circle', 'layer-ajax-form layui-btn');
            $this->addAction("一级{$this->mdl->cname}排序", array('Menu/sort', ['parent_id' => 1]), 'fa-sort', 'layer-ajax-form layui-btn-warm');
            $this->addAction("线型列表", array('Menu/lists', array('list' => 'list')), 'fa-eye','xxxx');
            $this->addAction("进入内容管理", array('Menu/content'), 'fa-reply', 'layui-btn-normal');
