...
class Database extends Run
{
    ...
    {  
        $this->notAccessAction = ['create', 'modify', 'batch_delete', 'sort'];
    ...
    public function lists()
    {
        ...
        $this->addAction('备份', 'javascript:void(0);', 'fa-exchange', 'backup layui-btn');
    ...
    public function delete()
    {
        ...
        if (!$time) {
            return $this->message('error', '请选择需要删除的文件', ['返回列表' => ['Database/filelist']]);
        }
        $db = new Backup();
        if ($db->delFile($time)) {
            return $this->message('success', '数据库备份文件删除成功', ['返回列表' => ['Database/filelist']]);
        }
    }
