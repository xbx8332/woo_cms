...
layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			...
			globalTabIdIndex++;
			var content = '<iframe width="100%" height="100%" style="min-width:400px;" scrolling="auto" src="' + data.href + '" data-id="' + globalTabIdIndex + '"></iframe>';
			var title = '';
            ...
            title += '<b class="lt"></b><b class="rt"></b>';
			//添加tab
            ...
            */
			...
			if(_config.closed) {
				...
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
                    ...
                    if (ELEM.titleBox.find('li.layui-this').length == 0) {
                        element.tabChange(ELEM.tabFilter, that.getTabId(ELEM.titleBox.find('li:last').find('cite').text()));
                    }
                    that.resize();
				});
			};
